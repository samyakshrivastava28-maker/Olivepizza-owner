...
const DAILY_SALES_HEADERS = [
  ...
  'CGST 2.5% (₹)', 'SGST 2.5% (₹)', 'TOTAL TAX 5% (₹)', 'DELIVERY (₹)',
...
async function runCompleteRebuild() {
  console.log('🚀 Starting Complete Google Sheets Rebuild & Data Restoration Engine...\n');
  ...
  // 2. CONNECT TO SPREADSHEET & ENSURE 12 TABS
  console.log(`\n🔨 Step 2: Connecting to Spreadsheet ${spreadsheetId} and Ensuring 12 Clean Structured Tabs...`);
  const sheets = getSheetsClient();
  const currentSheetRes = await sheets.spreadsheets.get({ spreadsheetId });
  const existingSheetTitles = (currentSheetRes.data.sheets || []).map(s => s.properties?.title || '');
  console.log('Existing Tabs:', existingSheetTitles);

  const desiredTabs = [
    'Dashboard', 'Monthly Summary', 'Daily Sales', 'Order Details',
    'Product Sales', 'Payments', 'GST & Tax', 'Discounts & Coupons',
    'Refunds & Cancellations', 'POS & Cashier Summary', 'Audit & Adjustments', 'Sync Status'
  ];
      ...
      addSheetRequests.push({
        addSheet: {
          properties: {
            title: tab,
            gridProperties: {
              rowCount: tab === 'Order Details' || tab === 'Product Sales' ? 5000 : 500,
              columnCount: 30,
              frozenRowCount: 1
            }
          }
  ...
  if (addSheetRequests.length > 0) {
    console.log(`Adding ${addSheetRequests.length} missing structured tabs...`);
  ...
  console.log('\n📊 Step 3: Populating Structured Headers & Formulas...');
  ...
  for (const p of headerPayloads) {
    try {
      ...
      console.warn(`Notice updating ${p.range}:`, e.message);
    }
  }

  // 4. RESTORE HISTORICAL DATA
  console.log('\n📥 Step 4: Restoring 80 Verified Historical Orders into Order Details (1 Order = 1 Row)...');
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: "'Order Details'!A2:AC5000"
  });
  ...
  console.log('📥 Step 5: Restoring Product Sales (1 Item = 1 Row)...');
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: "'Product Sales'!A2:O8000"
  });
  ...
  console.log('📥 Step 6: Restoring Payment Records...');
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: "'Payments'!A2:K5000"
  });
  ...
  // 5. UPDATE SYSTEM CONFIGURATION IN FIRESTORE
  ...
  await db.collection('franchise_sheets_metadata').doc(franchiseId).set({
    ...
    franchiseName,
    spreadsheetId,
    spreadsheetName: 'Olive Pizza — Rajnandgaon HQ Reports',
    spreadsheetUrl,
    status: 'CONNECTED',
    createdAt: now,
  ...
  console.log('\n🎉 ALL DATA REBUILT & MIGRATED SUCCESSFULLY!');
